...
/** วิเคราะห์สถานะ Power Factor และคืนค่าข้อความ + สถานะเตือน */
export function analyzePowerFactor(pf: number) {
  const threshold = 0.85;
  const isWarning = pf < threshold;
  return {
    isWarning,
    message: isWarning
      ? `ต่ำกว่าเกณฑ์ (ควร > ${threshold}) — แนะนำติดตั้ง capacitor bank`
      : "อยู่ในเกณฑ์ปกติ",
  };
}